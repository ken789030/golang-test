...
var showTable = function(label, titles, labels, dataJson, keys) {
    ...
    html += '<thead><tr>';
    for (i in titles) {
        html += '<th class="headers">' + titles[i] + '</th>';
    }
    html += '</tr></thead><tbody>';
    var key = '';
    for (j in keys) {
...
var showUserData = function(gameId, egretId, server, chanName, tag, chanId) {
    ...
    var url = "/Ajax/Stat/Game.getUserInfo?dataTag=" + tag + "&gameId=" + gameId + "&egretId=" + egretId + "&server=" + server + "&chanName=" + chanName + "&chanId=" + chanId;
